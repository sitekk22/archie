...
scripts="$DIST_ROOT/build_env/build_scripts"
echo "CHAPTER 7 DONE"
cp $scripts/prep-env.sh $scripts/post-chroot.sh $scripts/post-bash-login.sh $scripts/build_temp.sh $LFS/sources
cp -r $scripts/temp_tools $LFS/sources
cp -r $scripts/basic_software $LFS/sources
cp -r $scripts/sys_config $LFS/sources/
echo "SCRIPTS MOVED TO SOURCES"
